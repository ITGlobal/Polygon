...
namespace IBApi
{
    ...
    internal class Execution
    {
        ...
        public override string ToString() => "{ " + string.Join(", ", ToStringIterator());

        private IEnumerable<string> ToStringIterator()
        {
            yield return $"orderId:{orderId}";
            yield return $"clientId: {clientId}";
            yield return $"execId: '{execId}'";
            yield return $"time: '{time}'";
            yield return $"acctNumber: '{acctNumber}'";
            yield return $"exchange: '{exchange}'";
            yield return $"side: '{side}'";
            yield return $"shares: {shares}";
            yield return $"price: {price}";
            yield return $"permId: {permId}";
            yield return $"liquidation: {liquidation}";
            yield return $"cumQty: {cumQty}";
            yield return $"avgPrice: {avgPrice}";
            yield return $"orderRef: '{orderRef}'";
            yield return $"evRule: '{evRule}'";
            yield return $"evMultiplier: {evMultiplier}";
        }
    }
}
